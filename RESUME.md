...
**Live:** bleepboard.com — 600 sounds, 11 category hubs, 13 articles, 1,235 pages.
...
- ~~114 cached sounds awaiting copy~~ — **imported 2026-08-26.** 77 went live; the
  other 37 are held, see below.
...
- **37 held near-duplicates** in `data/deferred-duplicates.json` — needs an owner
  decision. Each is the same subject as a page we already have, with different
  audio, and 13 carry a character-for-character identical title ('X-Files Theme'
  vs 'X-Files Theme'). Publishing them would put two of our own pages in front of
  one query. Options: differentiate as variants, merge into the existing page as a
  second clip, or drop. Copy is written either way, so nothing is wasted.
- Enough adjacent material for a `/cartoon-soundboard/` hub with real headroom.
- Blog artwork is generated by `scripts/gen-blog-heroes.mjs` (`npm run gen:blog-heroes`).
  New articles need a `POSTS` entry there or they ship with no hero — the generator
  warns rather than failing the build. Never put a third-party character, logo or
  trade dress in one; there is a comment at the top of that file explaining why.
